            ...
            with ui.element("div").classes("col text-center"):
                with ui.row().classes("items-center justify-center no-wrap"):
                    ...
                    if _assign:
                        ...
                        ui.label(f"{t('assigned_to')}: {_assign}").classes(
...
async def setup_review():
    ...
    if not is_tour_completed():
        tour_video_id = queue_ids[initial_idx] if queue_ids else None
        has_ai = await run.io_bound(dp.has_model_annotations, tour_video_id)
